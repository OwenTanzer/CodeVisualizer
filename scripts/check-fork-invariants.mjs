...
const VSCODE_IMPORT = /(?:^|\n)\s*import[^;]*?from\s*['"]vscode['"]|require\(\s*['"]vscode['"]\s*\)/;
...
const LLM_REFERENCE = /['"][^'"]*\/llm\/[^'"]*['"]|\bLLMService\b|\bLLMManager\b/;
...
for (const [label, dir, exts] of [
  ...
  check(`@codevisualizer/core ${label} imports no vscode`, () => {
    ...
    if (files.length === 0) {
      ...
      throw new Error(`no ${exts.join('/')} files found under ${relative(repoRoot, dir)} -- build the core first`);
    }
  ...
  check(`@codevisualizer/core ${label} references no LLM module`, () => {
    ...
    if (offenders.length > 0) {
      throw new Error(`references an LLM module: ${offenders.map((f) => relative(repoRoot, f)).join(', ')}`);
    }
  });
}
...
check('LLMService dispatches anthropic to callAnthropic', () => {
  ...
  if (!/case\s+"anthropic":[\s\S]{0,200}?callAnthropic\s*\(/.test(src)) {
    throw new Error('the provider dispatch switch no longer routes "anthropic" to callAnthropic() -- this is the exact bug 353a00b fixed, where selecting Anthropic silently produced undefined');
  }
});
...
check('callAnthropic still calls the Anthropic Messages API', () => {
  ...
  if (!/https:\/\/api\.anthropic\.com\/v1\/messages/.test(src)) throw new Error('the Messages API endpoint is gone');
  if (!/["']anthropic-version["']/.test(src)) throw new Error('the required anthropic-version header is gone');
});
